...
class GeminiProvider {
    constructor() {
        this.client = new generative_ai_1.GoogleGenerativeAI(config_1.config.ai.apiKey);
        this.model = this.client.getGenerativeModel({ model: config_1.config.ai.model });
    }
    async generateJSON(prompt) {
        try {
            ...
            logger_1.logger.error('Gemini JSON generation error', { err });
...
`;
    ...
    async matchCandidateToJob(resumeText, jobDescription, jobSkills) {
        const prompt = `
...
`;
    ...
    async generateInterviewQuestions(candidateProfile, jobDescription) {
        const prompt = `
...
`;
    ...
    async copilotChat(message, context) {
        const systemContext = `
...
`;
        try {
            ...
            logger_1.logger.error('Copilot chat error', { err });
